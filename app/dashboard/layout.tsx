...
export default function ProtectedDashboard({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-canvas text-foreground">
      <p role={status === 'error' ? 'alert' : 'status'}>{status === 'error' ? error : 'Checking admin session…'}</p>
      {status === 'error' && <button className="rounded-xl bg-surface border px-4 py-2" onClick={() => {
        setStatus('checking');
